...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <div className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-white/95 dark:bg-[#0E2C33]/95 backdrop-blur-lg border-t border-slate-200/80 dark:border-[#1A4550] py-2 px-3 shadow-lg transition-colors">
      <div className="flex items-center justify-between max-w-md mx-auto">
        {/* 1. Home Tab (with pill when active as in screenshots) */}
        <button
          type="button"
          onClick={() => onNavigate('home')}
          className={`flex items-center gap-1.5 py-1.5 px-3.5 rounded-2xl transition-all cursor-pointer ${
            currentView === 'home'
